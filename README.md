...
## Customization with [BizOps Workspace](https://pubnub.com/docs/bizops-workspace/basics)

> Please use the **Chat application** to demonstrate and see the changes made through the User Management and Channel Management features of BizOps Workspace without a browser refresh.  Other apps within the showcase will not necessarily listen for App Context changes, for example the `Profile` screen requires a browser refresh to update the displayed data.  This is a limitation of the app, not a limitation of PubNub. 
...
**Deleting a Channel**: Deleting a channel will remove it from the list of public channels.  If the user was currently viewing the channel, another public channel will be loaded in its place.  As a reminder,  **if you delete all the channels and want to reset the Channel App Context in the keyset back to a default state, log out and log back into the showcase application.**
