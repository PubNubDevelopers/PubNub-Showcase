...
// The current location attachment
var locationAttachment = null;
...
async function loadChat () {
  ...
  try {
    var lat = localStorage.getItem('current-location-lat');
    var lng = localStorage.getItem('current-location-lng');
    var formatted_address = localStorage.getItem('current-location-formatted-address');
  ...
  try {
    if(currentLocation != null){
      channel = predefined_groups.groups[1].channel;
      await populateChatWindow(channel);
      await locationInputAttachment();
    }
    else{
...
async function locationInputAttachment(){
  function timeout(ms) {
  ...
  if(fileAttachment == null){
    ...
    var url = `https://maps.googleapis.com/maps/api/staticmap?center=${lat},${lng}&zoom=6&size=200x200&scale=1.5&markers=icon:${me.profileUrl}&key=AIzaSyDyl7ItKt5viBBju5Rwsqwrii5soyWUzp0`;
    document.getElementById('input-message').style.backgroundImage = 'url(' + url + ')';
    document.getElementById('input-message').value = `Hi, my location is ${formatted_address}`;
...
async function messageInputSend () {
  ...
  var fileUrl = null
  if (messageInput.style.backgroundImage != '' && fileAttachment != null) {
  ...
  if (messageText !== '' || fileUrl !== null || currentLocation !== null) {
    try {
...
function clearLocationSessionStorage(){
  localStorage.removeItem('current-location-lat');
  localStorage.removeItem("current-location-lng");
  localStorage.removeItem('current-location-formatted-address');
}
